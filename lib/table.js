...
Table.prototype.insert = function(data, next) {
  ...
  sql += values.join(",\n");
  sql += " RETURNING *";
  return this.db.query(sql, _.flatten(parameters, true), {single : returnSingle}, next);
};
...
Table.prototype.update = function(conditions, fields, next) {
  ...
  return this.db.query(sql, parameters, options, next);
};
...
Table.prototype.save = function(args, next){
  ...
  if(this.containsPk(args)){
    return this.update(clone(args),next);
  }else{
    return this.insert(args,next);
  }
};
...
Table.prototype.destroy = function(args, next){
  ...
  sql += " RETURNING *";
  return this.db.query(sql, where.params, next);
};
